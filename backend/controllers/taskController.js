...
import { sendTaskUpdateEmail } from '../utils/emailService.js';
  ...
  if (assignedUser && assignedUser.email) {
    await sendEmail(
      ...
      `Hi ${assignedUser.name},
       You have been assigned a new task titled <strong>"${title}"</strong>.
...
export const updateTask = async (req, res) => {
  try {
    ...
    await task.save();// ✅ Insert email notification here
    if (task.assignedTo) {
      ...
      if (assignedUser?.email) {
        await sendTaskUpdateEmail({
          ...
          taskLink: `http://localhost:5173/tasks/${task._id}`,
        });
      }
    }
